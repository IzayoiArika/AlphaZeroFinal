...
class SelfTrainer:
	...
	def train():
		...
		for i in range(1, config.Train.Times + 1):
			...
			sp.new_trial()
			if i >= 10 and i % 10 == 0:
				a0_model.train_model(config.Train.Epochs+1)
			
			if not i % config.Train.SnapshotGap:
				snapshot_path = os.path.join(models_dir, f'model_game_{i}.pth')
				torch.save(a0_model.state_dict(), snapshot_path)
				logging.info(f'保存模型快照: {snapshot_path}')    

				logging.info(f'缓存区数据量: {len(a0_model.buffers)}')
				with open('data/d1.json', 'w', encoding='utf-8') as f:
					json.dump(a0_model.buffers, f, ensure_ascii=False, indent=4)